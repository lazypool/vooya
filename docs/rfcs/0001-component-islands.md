# RFC 0001: Vooya Component Islands
...
application in another framework is usually disproportionate. Vooya investigates
...
Vooya components compile from Rust to `wasm32-unknown-unknown`. A host adapter
mounts one component into one host-owned element. Once mounted, Vooya owns every
...
```text
Host framework             Adapter                 Vooya runtime
--------------             -------                 ------------
...
- Vue slots, arbitrary host VNodes, `provide`/`inject`, or deep reactive object
  synchronisation.
- Server-side rendering, hydration, or a standalone Vooya application framework.
...
wrapper but does not own Vooya's Vite-oriented development workflow, incremental
