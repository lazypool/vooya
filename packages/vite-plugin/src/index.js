...
import { buildApplication, resolveRuntimeCrateRoot, resolveRustDependencyRoots, } from "./build-core.js";
import { createBuildScheduler } from "./build-scheduler.js";
import { compileVooStyle, generateVooDeclaration, generatedAdapterDefinition, generatedComponentBinding, parseVooComponent, } from "@vooya/compiler";
...
export function vooya({ framework = "vue", rust = {} } = {}) {
    ...
    const handleVooyaHotUpdate = ({ file }) => {
        if (!file.endsWith(componentExtension) &&
    ...
    return {
        ...
        resolveId(source, importer) {
            ...
            if (source.startsWith(stylePrefix))
                return `\0${source}`;
            if (!source.endsWith(componentExtension) || !importer)
                return null;
        ...
        load(id) {
            ...
            if (!id.endsWith(componentExtension))
                return null;
    ...
    return {
        ...
        complete() {
            if (startedAt !== undefined)
                logger?.info(`Vooya: Rust/WASM build complete in ${elapsed()}.`);
        },
        fail() {
            if (startedAt !== undefined)
                logger?.info(`Vooya: Rust/WASM build failed after ${elapsed()}.`);
        },
    };
}
