...
import { generateRustComponents, generateVooDeclaration, generatedAdapterDefinition, generatedComponentBinding, generatedComponentPrelude, parseVooComponent, } from "@vooya/compiler";
...
export function resolveRustDependencyRoots(rust = {}, applicationRoot) {
    if (!isPlainObject(rust.dependencies))
        return [];
...
export function buildApplication({ applicationRoot, components = [], rust = {}, runtimeCrateRoot = resolveRuntimeCrateRoot(), cacheRoot = resolve(applicationRoot, ".voo-cache"), outputDir = resolve(cacheRoot, "dist"), onRustBuildStart = () => { }, }) {
    ...
    writeIfChanged(resolve(cacheRoot, "Cargo.toml"), generatedCargoManifest({ applicationRoot, runtimeCrateRoot, rust }));
    writeIfChanged(resolve(cacheRoot, "src/lib.rs"), `pub use vooya_core::*;\n\n${generateRustComponents(components, sourcePaths)}`);
...
function generatePrecompiledVueDeclaration(component, manifest) {
    const declaration = generateVooDeclaration(component, "vue").replace("// Generated by @vooya/vite-plugin. Do not edit.", "// Generated by @vooya/vite-plugin/build. Do not edit.");
    return `${declaration}
...
function mergedWebSysFeatures(features = []) {
    if (!Array.isArray(features))
        throw new Error("Vooya rust.webSysFeatures must be an array.");
...
function generatedUserDependencies(dependencies = {}, applicationRoot) {
    ...
    return Object.entries(dependencies)
        ...
        if (reservedDependencies.has(name)) {
            throw new Error(`Rust dependency ${JSON.stringify(name)} is managed by Vooya and cannot be overridden.`);
        }
        return `${JSON.stringify(name)} = ${generatedDependencySpecification(name, specification, applicationRoot)}`;
    })
        .join("\n");
}
function generatedDependencySpecification(name, specification, applicationRoot) {
    ...
    if (unknown.length > 0) {
        throw new Error(`Rust dependency ${JSON.stringify(name)} has unsupported option ${JSON.stringify(unknown[0])}.`);
    }
    ...
    if (gitReferences.length > 0 && !specification.git) {
        throw new Error(`Rust dependency ${JSON.stringify(name)} option ${gitReferences[0]} requires git.`);
    }
    if (gitReferences.length > 1) {
        throw new Error(`Rust dependency ${JSON.stringify(name)} can use only one of branch, tag, or rev.`);
    }
    const values = [];
    ...
    if (specification.defaultFeatures !== undefined) {
        if (typeof specification.defaultFeatures !== "boolean") {
            throw new Error(`Rust dependency ${JSON.stringify(name)} option defaultFeatures must be a boolean.`);
        }
        values.push(`default-features = ${specification.defaultFeatures}`);
    }
    if (specification.features !== undefined) {
        if (!Array.isArray(specification.features) ||
            specification.features.some((feature) => typeof feature !== "string" || !feature)) {
            throw new Error(`Rust dependency ${JSON.stringify(name)} option features must be a string array.`);
        }
        values.push(`features = [${specification.features.map((feature) => JSON.stringify(feature)).join(", ")}]`);
    }
    return `{ ${values.join(", ")} }`;
}
...
export function remapRustDiagnostic(message, mappings) {
    ...
    for (const span of message.spans ?? []) {
        ...
        if (!mapping)
            continue;
        const sourceLine = mapping.startLine + span.line_start - 1 - (mapping.generatedLineOffset ?? 0);
        rendered = rendered.replaceAll(`${span.file_name}:${span.line_start}:${span.column_start}`, `${mapping.id}:${sourceLine}:${span.column_start}`);
        rendered = rendered.replace(new RegExp(`(\\n\\s*)${span.line_start}(\\s+\\|)`), `$1${sourceLine}$2`);
    }
    return rendered;
}
