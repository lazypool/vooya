 ...
 */
export function inspectToolchain({ env = process.env, run = runCommand, platform = hostPlatform(), home = homedir(), } = {}) {
    ...
    if (rustcVerbose?.value && isWindowsMsvcHost(rustcVerbose.value)) {
        ...
        results.push(check("MSVC linker link.exe", Boolean(linkerPath), linkerPath
            ? `found at ${linkerPath}`
    ...
    results.push(check(`Rust target ${WASM_TARGET}`, targetInstalled, targetInstalled
        ? "installed"
    ...
    results.push(check(`wasm-bindgen ${WASM_BINDGEN_VERSION}`, wasmBindgenMatches, wasmBindgenMatches
