...
export function generatedComponentBinding(component) {
    ...
    return {
        ...
        updateNames: Object.fromEntries(component.props.map((prop) => [prop.name, `voo_${stem}_update_${prop.name}`])),
...
function generateRustComponent(component, sourcePath) {
    ...
    const { exportName, disposeName, updateNames, propsName, eventsName, contextName } = generatedComponentBinding(component);
...
function generateEventDetail(parameters) {
    if (parameters.length === 0)
        return "let detail = wasm_bindgen::JsValue::UNDEFINED;";
    if (parameters.length === 1) {
        return `let detail = ${rustValueToJs(parameters[0].name, parameters[0].rustType)};`;
    }
    const pushes = parameters
        .map((parameter) => `detail.push(&${rustValueToJs(parameter.name, parameter.rustType)});`)
        .join("\n");
...
function rustValueToJs(name, rustType) {
    ...
    if (type === "number")
        return `wasm_bindgen::JsValue::from_f64(${name} as f64)`;
    if (type === "boolean")
...
function javascriptType(rustType) {
    ...
    if (rustType === "bool")
        return "boolean";
    if (rustType === "String" || rustType === "str" || rustType === "&str")
        return "string";
    throw new Error(`Unsupported Voo prop type "${rustType}".`);
}
function parseDefaultValue(source, rustType) {
    const type = javascriptType(rustType);
    if (type === "number" && /^-?(?:\d+\.?\d*|\.\d+)$/.test(source))
        return Number(source);
    if (type === "boolean" && /^(?:true|false)$/.test(source))
        return source === "true";
    if (type === "string" && /^"(?:[^"\\]|\\.)*"$/.test(source))
