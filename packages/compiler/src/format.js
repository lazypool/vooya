...
export function formatVooComponent(source, id = "<anonymous>.voo") {
    ...
    if (!componentBlock || !rustBlock)
        throw new Error("Required Voo blocks were not extracted.");
    assertNoTopLevelContent(source, [componentBlock, rustBlock, styleBlock].filter((block) => Boolean(block)), id);
    const sections = [
    ...
    if (component.style)
        sections.push("", `<style${component.style.scoped ? " scoped" : ""}>`, normalizeBlockContent(component.style.content), "</style>");
...
function extractBlock(source, tag, required, id) {
    ...
    if (matches.length > 1)
        throw new Error(`Duplicate <${tag}> block in ${id}.`);
    const [match] = matches;
...
function normalizeBlockContent(source) { return source.replace(/\r\n/g, "\n"); }
function appendBlank(lines) { if (lines.length > 0 && lines.at(-1) !== "")
    lines.push(""); }
