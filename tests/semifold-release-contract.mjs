...
try {
  ...
  assert.match(output, /已规划 7 个包|planned 7 package/i);
  for (const id of ["vooya-compiler", "vooya-core", "vooya-build-core", "vooya-vite-plugin", "vooya-vue", "vooya-react", "vooya-rspack"]) {
  ...
  for (const directory of ["compiler", "core", "build-core", "vite-plugin", "vue", "react", "rspack"]) {
  ...
  assert.equal(lockfile.packages["packages/vite-plugin"].dependencies["@vooya/build-core"], expectedVersion);
