...
test("runs filtering, sorting, virtualization, and measurement from VoyaGrid.voo", async ({
  page,
}) => {
  const browserErrors = [];
  ...
  await page.goto("/");
  const grid = page.getByRole("heading", { name: "Voya WASM island" }).locator("..");
  const summary = grid.getByRole("status");

  await expect(summary).toHaveText("100000 matching rows");
  await grid.getByLabel("Filter Voya rows").fill("item-0000");
  ...
  await grid.getByLabel("Filter Voya rows").fill("");
  const viewport = grid.locator("[data-voya-viewport]");
  await viewport.evaluate((element) => {
    element.scrollTop = 2_800;
    element.dispatchEvent(new Event("scroll"));
  });
  await expect(grid.locator("[data-voya-rows]")).toHaveAttribute(
    "style",
    /translateY\(2800px\)/,
  );
