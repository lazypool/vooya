  ...
  return new Promise((resolve, reject) => {
    ...
    child.stdout.on("data", (chunk) => {
      ...
      while (true) {
        ...
        if (!Number.isFinite(length) || buffer.length < boundary + 4 + length) return;
...
export function mapWorkspaceDiagnostic(diagnostic, extracted, workspace) {
  if (!diagnostic.range || diagnostic.range.start.line < workspace.generatedLineOffset) return undefined;
...
function parseContract(source) {
  ...
  if (block === undefined) throw new Error("Vooya bridge requires a <component> contract before <rust>.");
  const props = [];
  const events = [];
  let section;
  for (const raw of block.split(/\r?\n/)) {
    ...
    if (!line) continue;
    if (line === "props:" || line === "events:") { section = line.slice(0, -1); continue; }
    if (section === "props") {
      const match = /^([A-Za-z_]\w*)\s*:\s*(.+?)(?:\s*=\s*.+)?$/.exec(line);
      if (!match) throw new Error(`Unsupported Vooya prop contract: ${line}`);
      props.push({ name: match[1], type: match[2].trim() });
    } else if (section === "events") {
      const match = /^([A-Za-z_]\w*)\s*\((.*)\)$/.exec(line);
      if (!match) throw new Error(`Unsupported Vooya event contract: ${line}`);
      const parameters = match[2].trim() ? match[2].split(",").map((part) => {
        const parameter = /^\s*([A-Za-z_]\w*)\s*:\s*(.+?)\s*$/.exec(part);
        if (!parameter) throw new Error(`Unsupported Vooya event parameter: ${part}`);
